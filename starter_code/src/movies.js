...
const turnHoursToMinutes = arr => {
    // Make a deep clone, set three conditions to push diferent formats (h and min, h, min) into a new array
    ...
    newArr.forEach((movie, i) => {
        ...
        if (movie.duration.includes("h") && movie.duration.includes("min")) {
            durationArr.push(movie.duration.split("h"));
            durationArr[i][0] = durationArr[i][0] * 60;
            durationArr[i][1] = durationArr[i][1].slice(0, -3);
            durationArr[i] = durationArr[i].reduce(
                (acc, cur) => acc + Number(cur),
                0
            );
        } else if (movie.duration.includes("h")) {
            durationArr.push(Number(movie.duration.slice(0, -1)) * 60);
        } else {
            durationArr.push(Number(movie.duration.slice(0, -3)));
        }
    });

    // Use a loop to change prop duration old values to the new ones
    durationArr.forEach((time, i) => (newArr[i].duration = time));
    return newArr;
};
